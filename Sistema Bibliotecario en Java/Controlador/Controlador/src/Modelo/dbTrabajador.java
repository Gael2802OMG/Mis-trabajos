...
public class dbTrabajador extends dbManejador implements dbPersistencia {
    ...
    public void actualizar(Object objecto) throws Exception {
        if (objecto instanceof Trabajador) {
            Trabajador trabajador = (Trabajador) objecto;

            String consulta = "UPDATE biblioteca.personal SET nombre = ?, cargo = ?, correo = ?, tel_est = ?, direccion = ?, Estatus = ? WHERE Matricula = ?";
            if (this.conectar()) {
                try {
                    this.sqlConsulta = conexion.prepareStatement(consulta);
                    this.sqlConsulta.setString(1, trabajador.getNombre());
                    this.sqlConsulta.setString(2, trabajador.getCargo());
                    this.sqlConsulta.setString(3, trabajador.getCorreo());
                    this.sqlConsulta.setString(4, trabajador.getTel_est());
                    this.sqlConsulta.setString(5, trabajador.getDireccion());
                    this.sqlConsulta.setInt(6, trabajador.getEstatus());
                    this.sqlConsulta.setString(7, trabajador.getMatricula());
